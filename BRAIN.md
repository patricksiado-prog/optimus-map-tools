...
- 2026-06-02 — Created GHL Private Integration "claude11" for the Connect & Comm sub-account (location xZj500PjsflIQg2j9f9D). Token is stored ONLY in gitignored `ghl_ai/.env` (key GHL_TOKEN_CONNECT_COMM) — NOT in the repo, never commit it. Rotate it in GHL (Settings -> Private Integrations) if it is ever exposed. Frontline Direct (TXw28sw0Z2rl6tcCDhJY) stays on the session connector.
