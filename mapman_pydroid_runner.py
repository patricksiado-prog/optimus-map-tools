#!/usr/bin/env python3
"""
MapMan Pydroid Runner v3.3 - Resume Enabled (skips already-processed addresses)
FIX 2026-05-24: datetime.timezone bug. `from datetime import datetime` makes
datetime the CLASS, which has no .timezone attribute, so write_result() crashed
on EVERY row (nothing ever landed in the sheet). Now imports timezone directly.
"""
...
def get_already_done(client, sheet_id, tab):
    """Read output tab and return set of already-processed addresses."""
    try:
        ws = client.open_by_key(sheet_id).worksheet(tab)
        rows = ws.get_all_values()
        if len(rows) <= 1:
            return set()
        # Column 0 = Input Address
...
for i, item in enumerate(to_process, 1):
    addr = item["address"]
    print(f"\n[{i}/{len(to_process)}] {addr}")
    result = resolve(addr)
    write_result(out_ws, result)
    print(f"  -> {result['status']} | Phone: {result.get('phone') or 'N/A'} | Distance: {result.get('distance_m') or 'N/A'}m")
    time.sleep(0.2)

print(f"\nDone! Results in '{OUT_TAB}' tab.")
