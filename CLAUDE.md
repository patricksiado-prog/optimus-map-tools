...
# CURRENT STATE — updated 2026-09-06 00:1xZ (MAIN WORKBOOK IS SHRINKING — 8,484,584 → 8,261,881, −222,703 bytes at 00:13Z — the HP's scraper clean is running, FIRST PROOF 3ab3686 executes on a PC. Hunter on 67UOPK24 still paused 12h17m, needs Ctrl+Up. Runner deploy BLOCKED by a safety classifier (schtasks + cloud-file commands = malware shape); in-tools alternative offered. Earlier: PM EDITION SENT. BIGGEST FIND: THE BLAST IS TEXTING NEW YORK AND WASHINGTON STATE — outside the AT&T footprint entirely — and it is an installed MARKETPLACE APP `6a25617c1d5fcb2f6e8b826b`, not a GHL workflow. Opt-outs 8 → 12 today. THE OLD DRIVE INSTALL LINK IS CORRECT AGAIN — renamed back to INSTALL_OPTIMUS.bat, every URL it fetches VERIFIED 200 + current, and PATRICK HAS DOWNLOADED IT ON THE HP LAPTOP (photo: 7.0 KB = the right file). He is about to run it — that PC has no git and is on the 08-24 build, so the installer is the only thing that can move it. Both OPTIMUS LEADS state workbooks confirmed visible in his Drive. DEPLOYED 3ab3686, CDN-VERIFIED 21:37Z, brain-verify passes: five GHL tabs + LEGEND now build into TWO STATE WORKBOOKS, TX `1qMjCktl…` and NON-TX `1kE7Xsjc…`, every gold dot seeded, at the next scraper launch. Patrick: "we don't tweak shit after this." BOTH MACHINES STOPPED — hunter idle 8h, scraper feed 2h41m stale, both need a keypress not a reinstall. TWO blasts running, 11 opt-outs. THE SPAM SOLUTION is now a standing section below. Third sale found. Earlier: THE HUNTER WAS CAPTURING — a 5th PC, LAPTOP-67UOPK24, swept Angleton 77515 06:25-06:57 CT and the SPLIT WORKBOOK TOOK THE ROWS; the feed never saw it. Main workbook confirmed full. 2 WON in the pipeline. James blast still running. AM edition sent.)
...
### TWO TOKEN FILES GATE THE SCRAPER — AND ONE EXPLAINS THE STALE tabs.json (MEASURED off the HP console 2026-09-05 23:5xZ)

- **`github_token.txt` — MISSING. This is why `_feed/sheet/tabs.json` has been stuck at 2026-09-04.** Console: *"TAB COUNTS: 8 tabs -> NOT published -- no github_token.txt, counts stay stale on GitHub"*. Not a feed bug; the PC could not push. Searched in FIVE places (scraper lines 42-46): `~/Downloads/`, `~/Desktop/`, `~/`, `~/optimus/`, beside the script, cwd. **The PC that published on 09-04 already has one.**
- **`ghl_token.txt` — MISSING**, beside the scraper or `GHL_PIT_TOKEN`. Without it the five GHL tabs and both state workbooks are skipped.
- **THE SCRAPER'S PRINTED VERSION IS STALE AND CANNOT BE TRUSTED:** `VERSION = "2.1 (2026-06-18)"` is hard-coded at line 15 and has never been bumped, so the console says June while running 3ab3686. **Prove the build from behaviour instead** — "all 4707 rows are post-fix" and "junk tabs: none present" are post-09-03 only. **Offered, not pushed: bump VERSION on every scraper push like the hunter's BUILD_DATE.**
...
- **A THIRD SALE EXISTS AND IS NOT IN THE CRM: KATHLEEN WALSH / JOSEPH SANDEFUR, 2349 SE Hallahan St, Port Saint Lucie FL — Fiber 1 GIG, order `99-715950342717844`, INSTALL 9/9/2026 12-2PM.** Same household (`bigkandjoe@aol.com`), closed inside 24h off Rodelio's `fl_rodel_resi` blast. Opportunity `7LY88e6tw8wGmWyiDAB8` still sits at "Contacted" — **moving it to won is offered, not done.** That gap is why Rodelio cannot see install status.
- **THE 45 SCRIPTS ARE IN HIS GOOGLE DRIVE for Rodelio/Dave:** `1TukAcyh66RedAJ1mUvCQvWFnksjky33CBap7bGdct40` (no PII, no commission figures — safe to hand out). **I have NO filesystem access to any PC — the bridge is disconnected; DRIVE is the only folder I can write that reaches his machine.** Source of truth stays the repo file; regenerate the Drive copy when variants change.
- **45 MEASURED ONE-SEGMENT SMS VARIANTS in `templates/SMS_VARIANTS.md`** (green, gold, 2nd/3rd touch, business, replied-yes backlog, missed-call). Patrick: *"it worked more of that."*
- **PIPELINE READS 2 WON, 0 LOST — the true count is 3.** Shahrukh Majeed (211 Carey Ridge Ct, Fiber 1 GIG, 08-31) and Janell Dumas (Fiber 300, 08-29). The live alarm is now **0 lost** — a close rate needs a recorded loss.
- **BACKLOG: 24 `replied-yes` not closed, oldest 67 days** (2026-06-30). 3 of the 24 are texting opt-outs — call, never text.
- **THE "JAMES" BLAST IS RODELIO DELA CRUZ LEONIN** (`epWlnB5BMcpeUpMOMH5E`, `sakalamvillasis@gmail.com`, +63 PH phone) — **a person, not a rogue automation, so the fix is a conversation and better copy, not a workflow pause.** He is **`admin` on BOTH locations** with settings/workflows/triggers/bulk/team-management. His Frontline LC number is a **772** — that is why the blast targets the Treasure Coast. **Sandefur's `fl_rodel_resi` tag = his campaign, so it IS producing replies.** **Give him `templates/SMS_VARIANTS.md`** — 20 variants (10 green, 10 gold), each MEASURED at one segment.
- **The blast runs** from `+18506951985` — 8 STOPs in the 30 newest threads, five 772/561. **Template captured verbatim and MEASURED: 510 chars with non-GSM-7 bullets = NINE SEGMENTS** (BRAIN.md last section). Six defects: fake sender name, "installation team", signs **"Thanks, AT&T"** as the carrier, hand-written STOP that doubles GHL's, nine segments, and **"up to 4 months free" — UNVERIFIED and escalating** (the other template says 2; WebSearch found no AT&T free-months promo, only monthly discounts + free install). Its auto-replies also quote a flat `$30/month`.
- **CALL NOTES WRITTEN 18:2xZ with the full address as the first AND last line** on all four live leads (Barnes, Sandefur, Pope, Jacobs) — detail + note ids in BRAIN.md. **BARNES IS GOLD: `gold-attnet-confirmed` + `gold upgrade - never dialed` + a `jsbarnes1992@att.net` address = the provable att.net pool, and he has said in writing he will install. Best lead we own.** Barnes and Jacobs are BOTH Milton/Evergreen Estates.
...
- **THE BLAST IS TEXTING OUTSIDE THE AT&T FOOTPRINT (MEASURED 22:4xZ — the worst find of the day).** **Karen Duff `+1 516` = LONG ISLAND NY**, texted twice today from the LIVE number `+18502035831` (19:15Z + 19:17Z), replied STOP 21:28Z. **Jessica Stump `+1 253` = WASHINGTON STATE — a LUMEN state Patrick closed on 08-26.** Zero chance of revenue, pure spam complaint against numbers we need. **Every list build/scrub must now drop any area code outside the 21 AT&T states.**
- **THE BLAST IS AN INSTALLED MARKETPLACE APP, NOT A GHL WORKFLOW: `meta.marketplace.appId 6a25617c1d5fcb2f6e8b826b`.** That is why the from-number and copy match nothing in the account's workflows, and pausing workflows would never have stopped it. **First place to look.**
- **A BROKEN MERGE FIELD IS SHIPPING AS A TEXT:** Duff's 19:15:43Z message was, in full, `3464922635 / Reply STOP to unsubscribe. / Thanks, AT&T` — a bare phone number.
- **OPT-OUTS 8 → 12 in the 30 newest threads in one day.** Nephterline Louis complained rather than just opting out: *"I agree Stop this getting out of hand"*. Mix of 772/561 (Rodelio, FL) and 832/713 (TX) — **both blasts still running**.
- **FOUR off-book numbers now:** `346-492-2635` (advertised in blast copy), `832-247-4060` (the AI auto-responder quotes it in ~8 threads), `346-489-3489`, `346-590-6578`. **Live list re-read 22:4xZ = 11: 850-809-6942/203-5831/695-1985/789-6934/446-8236/318-4119 and 346-585-7098/585-2672/639-3567/659-2865/663-1038.**
- **A SECOND, WORSE BLAST WENT LIVE 14:18 CT** from `+18502035831` (a LIVE number): a **rocket-emoji "4 MONTHS of FREE"** text, **5 segments** because of that one emoji, signs as "AT&T", doubled STOP, and advertises **`346-492-2635` — a number NOT on the account**. **Scott Savage opted out 53 SECONDS after receiving it.** 3 new STOPs this hour, all TEXAS (Riaz, Savage, Wooley) — the earlier 8 were FLORIDA/Rodelio, so **TWO blasts are running**. Opt-outs 8 → 11 in three hours.
- **THREE off-book numbers now, none in the 11 live:** `+13464893489` (places calls silently) and `+13465906578` (fails SMS loudly, "Number not available in account"). **Something in the send path holds released numbers and tags contacts as texted anyway.** The audit — how many `fiber-sms-sent` contacts' only text failed — is offered and NOT yet run.
...
- **gspread-6 bug**: scraper's split-workbook redirect AND the Enriched Leads board are dead on every PC. Fix tested in `patches/gspread6-client/`, **UNPUSHED (RULE 0)**. Hunter is clean.
- **Gold + grey have NEVER been deduped**: fix tested in `patches/dedupe-gold-grey/`, **UNPUSHED (RULE 0)**. 176 readable gold rows = 10 addresses; 7631 Fuqua (×96) is not a real property.
- **The only provable gold is the 192 att.net/sbcglobal rows in GHL** (`GOLD_CHUNKS/GOLD_1_of_5.csv`, `GOLD_2`). The tab's `Build Code` is empty on every readable row; `py decode_gold.py` on the PC is the answer, not a code change.
- **Milton / Evergreen Estates = 117 doors, we hold 141, 133 callable** (`MILTON_NEW_FIBER_callable.csv`). **98 re-texted today 12:27 CT from the six FL 850s; Patrick chose to HOLD them in the dialer.** Milton is on none of the 8 tabs.
- **12 outbound numbers now (6×TX 346, 6×FL 850), fifth swap** — read `ghl_list_phone_numbers` every time. Account hit a provider opt-out throttle 09-03; 10 STOPs in the last 30 conversations.
- **RESI is the product; `Fiber Green Biz` is the new-fiber COMPASS, not a call list.** 7,300 on the tab, 3,767 in GHL, 89% already in the alpha pool. `Upgrade Orange Biz` = 62 = the gold half of the detector is dead until a scraper launch with room to write.
