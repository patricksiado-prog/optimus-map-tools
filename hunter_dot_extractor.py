...
def fetch_image_from_drive(file_id, token):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    try:
        r = requests.get(dl_url, headers=headers, timeout=60, verify=False)
    except requests.exceptions.ReadTimeout:
        print("  SKIP - Drive timeout")
        return None
    except Exception as e:
...
PROCESSED_IDS_FILE = os.path.join(os.path.expanduser("~"), "Desktop", "processed_ids.json")

def load_processed_ids():
    try:
        if os.path.exists(PROCESSED_IDS_FILE):
            return set(json.load(open(PROCESSED_IDS_FILE)))
    except Exception:
        pass
    return set()
...
def mark_processed(file_id, fn):
    ids = load_processed_ids()
    ids.add(file_id)
    try:
        json.dump(list(ids), open(PROCESSED_IDS_FILE, "w"))
    except Exception:
        pass
    ...
    for i, f in enumerate(files, 1):
        ...
        try:
            ...
            if r.get("status") != "OK":
                ...
                print(f"  G={r['green_count']} O={r['orange_count']} WRITTEN={r.get('written', 0)}")
                for row in r.get("results", []):
                    ...
                    if ptype == "COMMERCIAL":
                        ...
                        res_count += 1
                    print(f"    {row['color']}: {row['address']} ({ptype})")
            ...
            if newly_processed % 10 == 0:
                post_status(f"[{i}/{total}] Comm:{comm_count} Res:{res_count}")

        except Exception as e:
            print(f"  FATAL on {fn}: {e}")
            post_status(f"ERROR on {fn}: {e}")
            continue

    print("\n" + "=" * 70)
    print("COMPLETE")
    print(f"  Total: {total}")
    print(f"  Already done: {already_skipped}")
    print(f"  New this run: {newly_processed}")
